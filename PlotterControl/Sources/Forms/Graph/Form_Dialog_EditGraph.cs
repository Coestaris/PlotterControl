    ...
    public partial class Form_Dialog_EditGraph : Form
    {
        ...
        private void button_data_formula_compile_Click(object sender, EventArgs e)
        {
            ...
            if (GlobalErrorsLims !=null || GlobalErrors != null)
            {
                ...
                label_data_formula_status.Text = string.Format("?????????????? {0} ????????????.", (GlobalErrors?.Count == null ? 0 : GlobalErrors?.Count) + (GlobalErrorsLims?.Count == null ? 0 : GlobalErrorsLims?.Count));
            ...
            else
            {
                ...
                label_data_formula_status.Text = string.Format("OK! ?????????????? ???? {0} ??????????????????????.", (DateTime.Now - startTime).TotalMilliseconds);
        ...
        private void button_data_formula_status_more_Click(object sender, EventArgs e)
        {
            ...
            {
                res += string.Format(messages[3], "??????????????", 2);
                res += string.Format(messages[3], "????????????????", 3);
            }
            else
            {
                if (GlobalErrorsLims.FindAll(p => p.StartsWith("/")).Count() == 0) res += string.Format(messages[3], "??????????????", 2);
                else
                {
                    var a = GlobalErrorsLims.FindAll(p => p.StartsWith("/")).ToArray();
                    res += string.Format(messages[4], a.Length, "??????????????", CurrentCompStringLow, string.Join("\n", a.Select(p => "   - " + p.Trim('/'))), ds.LowLimFormula, 2);
                }
                if (GlobalErrorsLims.FindAll(p => p.StartsWith("\\")).Count() == 0) res += string.Format(messages[3], "????????????????" ,3);
                else
                {
                    var a = GlobalErrorsLims.FindAll(p => p.StartsWith("\\")).ToArray();
        ...
        private void Form_Dialog_EditGraph_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (Object.DataSource.GetType() == typeof(FormulaDataSource))
            {
                ...
                */
                ...
                if (CurrentCompString != ds.Formula) res += string.Format("?????????????????? ?????????????? \"{0}\" ???? ?????????????????? ?? ?????????????? \"{1}\"", CurrentCompString, ds.Formula);
                if (CurrentCompStringHigh != ds.HighLimFormula) res += string.Format("?????????????????? ?????????????? ?????????????? ?????????????? ?????????????? ?????????????? \"{0}\" ???? ?????????????????? ?? ?????????????? \"{1}\"\n", CurrentCompStringLow, ds.HighLimFormula);
                if (CurrentCompStringLow != ds.LowLimFormula) res += string.Format("?????????????????? ?????????????? ?????????????? ???????????? ?????????????? ?????????????? \"{0}\" ???? ?????????????????? ?? ?????????????? \"{1}\"\n", CurrentCompStringHigh, ds.LowLimFormula);

                if (textBox_markers_period.Text != Object.Markers.PeriodFormula) res += string.Format("?????????????????? ?????????????? ?????????????? ???????????????? \"{0}\" ???? ?????????????????? ?? ?????????????? \"{1}\"\n", textBox_markers_period.Text, Object.Markers.PeriodFormula);
                if (textBox_markers_period_start.Text != Object.Markers.LowLimFormula) res += string.Format("?????????????????? ?????????????? ???????????? ?????????????? ???????????????? \"{0}\" ???? ?????????????????? ?? ?????????????? \"{1}\"\n", textBox_markers_period_start.Text, Object.Markers.LowLimFormula);
                if (textBox_markers_period_end.Text != Object.Markers.HighLimFormula) res += string.Format("?????????????????? ?????????????? ???????????? ?????????????? ???????????????? \"{0}\" ???? ?????????????????? ?? ?????????????? \"{1}\"\n", textBox_markers_period_end.Text, Object.Markers.HighLimFormula);

                if(res.Length != 0) MessageBox.Show(res, "????????????", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void button_markers_status_Click(object sender, EventArgs e)
        {
            ...
            if (lowErrors.Count == 0) res += string.Format(messages[3], "??????????????", 2);
            else res += string.Format(messages[4], lowErrors.Count, "??????????????", textBox_markers_period_end.Text, string.Join("\n", lowErrors.Select(p => "   - " + p.Trim('/'))), Object.Markers.LowLimFormula, 2);

            if (highErrors.Count == 0) res += string.Format(messages[3], "????????????????", 3);
            else res += string.Format(messages[4], highErrors.Count, "????????????????", textBox_markers_period_start.Text, string.Join("\n", highErrors.Select(p => "   - " + p.Trim('\\'))), Object.Markers.HighLimFormula, 3);
        ...
        private void button_markers_compile_Click(object sender, EventArgs e)
        {
            ...
            if (Object.Markers.UsePeriodic)
            {
                ...
                if (GlobalErrorsPeriod != null)
                {
                    ...
                    label_markers_status.Text = string.Format("?????????????? {0} ????????????.", (GlobalErrorsPeriod?.Count == null ? 0 : GlobalErrorsPeriod?.Count));
                ...
                else
                {
                    ...
                    label_markers_status.Text = string.Format("OK! ?????????????? ???? {0} ??????????????????????.", (DateTime.Now - startTimePeriod).TotalMilliseconds);
