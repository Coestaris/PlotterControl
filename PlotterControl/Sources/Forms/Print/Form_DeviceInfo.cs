...
namespace CnC_WFA
{
    public partial class Form_DeviceInfo : Form
    {
        ...
        private void Form_DeviceInfo_Load(object sender, EventArgs e)
        {
            ...
            label1.Text = string.Format(TB.L.Phrase["Form_DeviceInfo.Tip"], di.FreeSpace, (int)di.CardSize - (int)di.FreeSpace);
            lstValues = new List<int>() { (int)di.FreeSpace, (int)di.CardSize - (int)di.FreeSpace };
            richTextBox1.Text = string.Format(TB.L.Phrase["Form_DeviceInfo.MainData"], di.ToString(), master.Device.DeviceInfo.ToString());
            createPie();
        }
