        ...
        private void TrySetupFolder()
        {
              ...
              catch (WrongPacketInputException ex)
            {
                if (System.Windows.Forms.MessageBox.Show(
                            string.Format("???????????????????? ???????????????? ????????????. ?????????????????? ???????????? ???????? WrongPacketInputException (?????????????? {0}), ?????? ?????????? ???????????????? ?????? ???????????????????? ???????????????? ???? ???????????????? ?? ???? ???????????????? ???????????????????????? ???????????????? ?? ?????????????????? ????????????. ???????????????????? ?????????????????????????? ?????? ?? ???????????? \"????????????\"", ex.Type.ToString()), "Error", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error) == DialogResult.Retry)
                    TrySetupFolder();
        ...
        private void listView1_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (listView1.SelectedItems.Count == 1)
            {
                if (listView1.SelectedItems[0].SubItems[1].Text != "<folder>")
                    ...
                    {
                        if(System.Windows.Forms.MessageBox.Show("???? ???????????? ???????????????? ???????? ???? ???? ?? ?????????????????????? ???????", "????????????????", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            string path = string.Join("", CurrentPath);
